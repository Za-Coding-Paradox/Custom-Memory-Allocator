...
// Context & Stats
// ============================================================================
enum class AllocatorContextType : uint8_t {
  Frame,
  Level,
  Global,
  Temporary,
...
public:
  explicit AllocatorEngine(size_t SlabSize = g_ConstSlabSize,
                           size_t ArenaSize = g_ConstArenaSize,
  ...
  Allocate(size_t Size,
    ...
    static_assert(std::is_same_v<TContext, FrameLoad> ||
                      std::is_same_v<TContext, LevelLoad> ||
                      std::is_same_v<TContext, GlobalLoad>,
                  "Invalid Allocator Context Type");

    if (!m_IsInitialized.load(std::memory_order_acquire)) [[unlikely]] {
      // Warning logic could be moved to .cpp helper, but keeping inline for
      // speed
      return nullptr;
    }

    void *ptr = LinearStrategyModule<TContext>::Allocate(Size, Alignment);

    if (ptr != nullptr) {
      AllocatorContextType contextType = GetContextType<TContext>();
      // RecordAllocation is defined in .cpp, but safe to call here
      m_ContextStats[contextType].RecordAllocation(Size);
    }
    ...
    requires(!TContext::IsRewindable)
  {
    ...
    AllocatorContextType contextType = GetContextType<TContext>();
    m_ContextStats[contextType].RecordReset();
  }
  ...
  template <typename T>
  [[nodiscard]] T *ResolveHandle(TypedHandle<T> Handle) const noexcept {
  ...
  public:
    ScopedAllocation(AllocatorEngine &Engine, size_t Count = 1) noexcept
        : m_Engine(Engine),
          m_Handle(Engine.AllocateWithHandle<T, TContext>(Count)) {}
  ...
  template <typename T, typename TContext>
  [[nodiscard]] ScopedAllocation<T, TContext>
  AllocateScoped(size_t Count = 1) noexcept {
  ...
  // ========================================================================
  [[nodiscard]] const AllocationStats &
  GetStats(AllocatorContextType Context) const noexcept;
...
private:
  ...
  template <typename TContext>
  [[nodiscard]] static AllocatorContextType GetContextType() noexcept {
  ...
  GetContextName(AllocatorContextType Type) noexcept;
};
