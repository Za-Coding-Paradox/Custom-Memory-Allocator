...
// ============================================================================
// AllocationStats Implementation
// ============================================================================
void AllocationStats::RecordAllocation(size_t Size) noexcept {
  AllocationCount.fetch_add(1, std::memory_order_relaxed);
  size_t current =
      TotalAllocated.fetch_add(Size, std::memory_order_relaxed) + Size;

  // CAS loop for Peak
  size_t peak = PeakAllocated.load(std::memory_order_relaxed);
  while (current > peak && !PeakAllocated.compare_exchange_weak(
...
// ============================================================================
// AllocatorEngine Implementation
// ============================================================================

AllocatorEngine::AllocatorEngine(size_t SlabSize, size_t ArenaSize,
                                 uint32_t HandleCapacity) noexcept
    : m_Registry(SlabSize, ArenaSize), m_HandleTable(HandleCapacity),
      m_SlabSize(SlabSize), m_ArenaSize(ArenaSize) {
...
AllocatorEngine::GetStats(AllocatorContextType Context) const noexcept {
...
void AllocatorEngine::PrintStatistics() const noexcept {
  ...
  LOG_ALLOCATOR("INFO",
                "  Utilization: " << (m_HandleTable.GetUtilization() * 100.0f)
                                  << "%");
}
...
const SlabRegistry &AllocatorEngine::GetRegistry() const noexcept {
  return m_Registry;
}

HandleTable &AllocatorEngine::GetHandleTable() noexcept {
  return m_HandleTable;
}

const HandleTable &AllocatorEngine::GetHandleTable() const noexcept {
  return m_HandleTable;
}
...
const char *
AllocatorEngine::GetContextName(AllocatorContextType Type) noexcept {
  switch (Type) {
