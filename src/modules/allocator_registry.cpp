...
bool SlabRegistry::InitializeArena() noexcept
{
    ...
    if (m_ArenaRegistryStart == MAP_FAILED) [[unlikely]] {
        m_ArenaRegistryStart = nullptr;
        return false;
    }

    const size_t UnitSize = sizeof(SlabDescriptor) + m_SlabSize;
    m_DescriptorCount = m_ArenaSize / UnitSize;

    if (m_DescriptorCount == 0) [[unlikely]]
        return false;
...
{
    if (!SlabToFree) [[unlikely]]
        return;

    const ptrdiff_t Idx = SlabToFree - m_DescriptorSpan.data();
    if (Idx < 0 || static_cast<size_t>(Idx) >= m_DescriptorCount) [[unlikely]]
        return;
