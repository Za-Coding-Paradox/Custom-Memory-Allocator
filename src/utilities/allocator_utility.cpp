...
[[nodiscard]] auto Utility::AlignForward(void *Address,
                                         size_t Alignment) noexcept -> void * {
  // FIX (Bug #6): Validate alignment is power of 2
  assert(IsPowerOfTwo(Alignment) &&
         "AlignForward: Alignment must be power of 2");
...
[[nodiscard]] auto Utility::AlignBackward(void *Address,
                                          size_t Alignment) noexcept -> void * {
  // FIX (Bug #6): Validate alignment is power of 2
  assert(IsPowerOfTwo(Alignment) &&
         "AlignBackward: Alignment must be power of 2");
...
[[nodiscard]] auto Utility::Add(void *Address, size_t Offset) noexcept
    -> void * {
  // FIX (Bug #10): Validate no overflow
  const auto AddressVal = std::bit_cast<uintptr_t>(Address);
  const auto OffsetVal = static_cast<uintptr_t>(Offset);

  assert(AddressVal <= (UINTPTR_MAX - OffsetVal) &&
         "Utility::Add: Pointer arithmetic overflow");

  return std::bit_cast<void *>(AddressVal + OffsetVal);
}

[[nodiscard]] auto Utility::Subtract(void *Address, size_t Offset) noexcept
    -> void * {
  // FIX (Bug #10): Validate no underflow
  const auto AddressVal = std::bit_cast<uintptr_t>(Address);
  const auto OffsetVal = static_cast<uintptr_t>(Offset);

  assert(AddressVal >= OffsetVal &&
         "Utility::Subtract: Pointer arithmetic underflow");

  return std::bit_cast<void *>(AddressVal - OffsetVal);
}

[[nodiscard]] auto Utility::GetPadding(void *Address, size_t Alignment) noexcept
    -> size_t {
  const auto AddressVal = std::bit_cast<uintptr_t>(Address);
  const auto AlignmentMask = static_cast<uintptr_t>(Alignment - 1);
...
[[nodiscard]] auto Utility::GetPaddingWithHeader(void *Address,
                                                 size_t Alignment,
...
[[nodiscard]] auto Utility::PointerDifference(void *Address1,
                                              void *Address2) noexcept
    -> size_t {
  return static_cast<size_t>(std::bit_cast<uintptr_t>(Address1) -
                             std::bit_cast<uintptr_t>(Address2));
}

[[nodiscard]] auto Utility::GetRelativeAddress(void *Address,
                                               void *ArenaStart) noexcept
    -> size_t {
  return static_cast<size_t>(std::bit_cast<uintptr_t>(Address) -
                             std::bit_cast<uintptr_t>(ArenaStart));
}

[[nodiscard]] auto Utility::GetSlabIndex(void *Address, void *ArenaStart,
                                         size_t SlabSize) noexcept -> size_t {
  // FIX (Bug #7): Validate no underflow
  const auto AddressVal = std::bit_cast<uintptr_t>(Address);
  const auto StartVal = std::bit_cast<uintptr_t>(ArenaStart);

  assert(AddressVal >= StartVal &&
         "GetSlabIndex: Address is before ArenaStart");
...
[[nodiscard]] auto Utility::GetSlabStart(size_t SlabIndex, void *ArenaStart,
  ...
  const auto RelativeOffset =
      static_cast<uintptr_t>(SlabIndex) * static_cast<uintptr_t>(SlabSize);
...
[[nodiscard]] auto Utility::PackHandle(size_t Index, size_t Generation) noexcept
    -> size_t {
  return (Generation << g_GenerationShiftKey) | (Index & g_MaximumBitMask);
}

[[nodiscard]] auto Utility::UnpackHandleIndex(size_t HandleID) noexcept
    -> size_t {
  return HandleID & g_MaximumBitMask;
}

[[nodiscard]] auto Utility::UnpackHandleGeneration(size_t HandleID) noexcept
    -> size_t {
